<script setup lang="js">
    ...
    // use current route to get the house id
    const route = useRoute()
    // use houses store to control the page
    const store = useHouses()

    // load the house when page is loaded for the first time
    onMounted(() => {
        store.loadOne(route.params.id)
    })

    // if we are already in the detail page
    // and user clicks on the recommended house list
    // then we need to use this hook to load the house because the component is already mounted
...
<template>
    <div v-if="store.house" class="header">
        ...
        <div class="toolbar">
            <BaseBackButton to="/house">
                <span class="back-button-label"> Back to overview </span>
            </BaseBackButton>
            <div class="toolbar-house-actions">
                <HouseActions
                    v-if="store.house.madeByMe"
                    :id="store.house.id"
                />
            </div>
        </div>
    ...
    <div class="house-detail-page">
        ...
        <div class="recommended">
            ...
            <HouseItem
                v-for="house of store.recommendedHouses"
                :house="house"
            />
        </div>
    </div>
